...
document.querySelector('#citySelect').addEventListener('change', async evt => {
  ...
  try {
    ...
    document.querySelector('#apparentTemp div:nth-child(2)')
      ...
      document.querySelector('#todaysStats div:nth-child(2)')
      .textContent = `Precipitation: ${data.current.precipitation_probability * 100}%`
      document.querySelector('#todaysStats div:nth-child(3)')
      .textContent = `Humidity: ${data.current.humidity}%`
      document.querySelector('#todaysStats div:nth-child(4)')
      .textContent = `Wind: ${data.current.wind_speed}m/s`
  
data.forcast.daily.foreach.daily.forEach((day, idx) => {
  ...
  apparent.textContent = descriptions.find(d =>[0] === day.weather_description)[1]
